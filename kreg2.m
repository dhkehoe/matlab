...
if numel(size(d)) ~=2 || size(d,2) ~= 3
    error('Input ''d'' must be an N x 3 matrix');
end
...
switch p.kernel
    case 1 % Gaussian
        k = @(x1,x2,bw) exp( -x1.^2/(2*bw(1)^2) -x2.^2/(2*bw(2)^2) );
    case 2 % Rectangular
        k = @(x1,x2,bw) ( -bw(1)/2 <= x1 & x1 <= bw(1)/2 & -bw(2)/2 <= x2 & x2 <= bw(2)/2 );
    case 3 % von Mises
        k = @(x1,x2,bw) exp( bw(1)*cos(x1)+bw(2)*cos(x2) );
end
